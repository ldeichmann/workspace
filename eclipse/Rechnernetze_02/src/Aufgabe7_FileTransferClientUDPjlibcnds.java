...
public class Aufgabe7_FileTransferClientUDPjlibcnds {
    ...
    public static void main(String args[]) throws Exception{ 
	    ...
	    while ((len = fr.read(buf, 0, buf.length)) != -1) {
			...
			while (true) {
				try {
			        DatagramPacket packet = new DatagramPacket(Tools.concatByteArrays(Tools.addParityBytes(buf), currBit), buf.length + 1 + currBit.length);
					...
					System.out.println("Roundtrip took: " + (System.currentTimeMillis() - temp) + "ms" );
 					break;
				} catch (Exception e) {
					loss++;
				}
